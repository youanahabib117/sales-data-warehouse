...
SELECT
    ...
    sls_quantity,
    sls_price,
    (s.sls_sales / s.sls_quantity) AS price_per_item,
    COALESCE(p.prd_cost, e.prd_cost) AS cost_per_item
FROM relational.crm_sales s
LEFT JOIN relational.stg_products_dim p 
ON s.sls_prd_key = p.prd_key
AND s.sls_order_dt >= p.prd_start_date
AND (s.sls_order_dt <= p.prd_end_date OR p.prd_end_date IS NULL)
LEFT JOIN earliest_cost e
ON s.sls_prd_key = e.prd_key
)
SELECT
