...
-- 3. Sales year-by-year analysis
SELECT 
    DATE_PART('year', order_date) AS "Year",
    SUM(sales_total) AS "Total Sales",
    ROUND((SUM(sales_total)-LAG(SUM(sales_total)) OVER (ORDER BY DATE_PART('year', order_date)))::numeric/LAG(SUM(sales_total)) OVER (ORDER BY DATE_PART('year', order_date)) * 100, 2) AS "Year-Over-Year Growth (%)",
    SUM(SUM(sales_total)) OVER (ORDER BY DATE_PART('year', order_date)) AS "Running Total"
FROM analytics.sales_fact
WHERE DATE_PART('year', order_date) BETWEEN 2011 AND 2013
GROUP BY "Year"
ORDER BY "Year";

-- 4. Sales month-by-month analysis
SELECT 
    ...
    SUM(SUM(sales_total)) OVER (ORDER BY DATE_PART('year', order_date), DATE_PART('month', order_date)) AS "Running Total"
FROM analytics.sales_fact
WHERE DATE_PART('year', order_date) BETWEEN 2011 AND 2013
