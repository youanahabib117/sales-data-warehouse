...
SELECT 
    SUM(sales_total) AS total_sales,
    SUM((price_per_item - cost_per_item)*quantity) AS total_margin,
    ROUND((SUM((price_per_item - cost_per_item)*quantity)::numeric/SUM(sales_total))*100.0, 2) AS gross_margin_pct
FROM analytics.sales_fact
...
SELECT
    ...
    SUM((s.price_per_item - s.cost_per_item)*s.quantity) AS total_margin,
    ROUND((SUM((s.price_per_item - s.cost_per_item)*s.quantity)::numeric/SUM(s.sales_total))*100.0, 2) AS gross_margin_pct
FROM products p
...
SELECT
    p.prd_key,
    p.cat,
    ROUND(AVG(s.price_per_item - s.cost_per_item), 2) AS avg_profit,
    ROUND(AVG(s.price_per_item), 2) AS avg_price,
    ROUND((AVG(s.price_per_item - s.cost_per_item)::numeric/AVG(s.price_per_item))*100.0, 2) AS gross_margin_pct
FROM products p
LEFT JOIN analytics.sales_fact s
ON p.prd_key = s.prd_key
GROUP BY p.prd_key, p.cat
...
SELECT
    ...
    SUM((price_per_item-cost_per_item)*quantity) AS "Total Gross Profit",
    ROUND((SUM((price_per_item-cost_per_item)*quantity)-LAG(SUM((price_per_item-cost_per_item)*quantity)) OVER (ORDER BY DATE_PART('year', order_date)))::numeric/LAG(SUM((price_per_item-cost_per_item)*quantity)) OVER (ORDER BY DATE_PART('year', order_date)) * 100, 2) AS "Year-Over-Year Profit Growth (%)",
    ROUND((SUM((price_per_item-cost_per_item)*quantity)::numeric/SUM(sales_total) * 100.0) - LAG(SUM((price_per_item-cost_per_item)*quantity)::numeric/SUM(sales_total) * 100.0) OVER (ORDER BY DATE_PART('year', order_date)), 2) AS "Margin Change (%)"
FROM analytics.sales_fact
...
SELECT 
    ...
    SUM((price_per_item-cost_per_item)*quantity) AS "Total Gross Profit",
    ROUND((SUM((price_per_item-cost_per_item)*quantity)-LAG(SUM((price_per_item-cost_per_item)*quantity)) OVER (ORDER BY DATE_PART('year', order_date), DATE_PART('month', order_date)))::numeric/LAG(SUM((price_per_item-cost_per_item)*quantity)) OVER (ORDER BY DATE_PART('year', order_date), DATE_PART('month', order_date)) * 100, 2) AS "Profit Growth (%)",
    ROUND((SUM((price_per_item-cost_per_item)*quantity)::numeric/SUM(sales_total) * 100.0) - LAG(SUM((price_per_item-cost_per_item)*quantity)::numeric/SUM(sales_total) * 100.0) OVER (ORDER BY DATE_PART('year', order_date), DATE_PART('month', order_date)), 2) AS "Margin Change (%)"
FROM analytics.sales_fact
WHERE DATE_PART('year', order_date) BETWEEN 2011 AND 2013
